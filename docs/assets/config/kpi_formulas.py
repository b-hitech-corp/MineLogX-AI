...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["distance_km"].sum(), df["fuel_litres"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["active_hours"].sum(), df["scheduled_hours"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "available_hours", "scheduled_hours") or
        _div(df["available_hours"].sum(), df["scheduled_hours"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "cycle_time_min") or
        round(float(df["cycle_time_min"].mean()), 2)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "payload_tonnes", "operating_hours") or
        _div(df["payload_tonnes"].sum(), df["operating_hours"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "payload_tonnes", "target_payload_tonnes") or
        _div(df["payload_tonnes"].sum(), df["target_payload_tonnes"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "fuel_litres", "operating_hours") or
        _div(df["fuel_litres"].sum(), df["operating_hours"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["total_cost"].sum(), df["distance_km"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["idle_hours"].sum(), df["engine_on_hours"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["on_time_deliveries"].sum(), df["total_deliveries"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["operating_hours"].sum(), max(df["failure_count"].sum(), 1))
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "repair_time_hours", "failure_count") or
        _div(df["repair_time_hours"].sum(), max(df["failure_count"].sum(), 1))
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "downtime_hours", "scheduled_hours") or
        _div(df["downtime_hours"].sum(), df["scheduled_hours"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "pm_completed", "pm_scheduled") or
        _div(df["pm_completed"].sum(), df["pm_scheduled"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["on_time_services"].sum(), df["total_scheduled_services"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "oil_samples_taken", "oil_samples_due") or
        _div(df["oil_samples_taken"].sum(), df["oil_samples_due"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "fatigue_events", "operating_hours") or
        _div(df["fatigue_events"].sum(), df["operating_hours"].sum(), 1000)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "speed_violations", "distance_km") or
        _div(df["speed_violations"].sum(), df["distance_km"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "seatbelt_compliant_time", "engine_on_hours") or
        _div(df["seatbelt_compliant_time"].sum(), df["engine_on_hours"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "near_misses", "operating_hours") or
        _div(df["near_misses"].sum(), df["operating_hours"].sum(), 1000)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "unsafe_events", "operating_hours") or
        _div(df["unsafe_events"].sum(), df["operating_hours"].sum(), 1000)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        ...
        _div(df["co2_grams"].sum(), df["distance_km"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "co2_grams", "tonnes_moved") or
        _div(df["co2_grams"].sum() / 1000, df["tonnes_moved"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "dust_readings", "dust_threshold_breaches") or
        _div(
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "water_litres", "tonnes_moved") or
        _div(df["water_litres"].sum(), df["tonnes_moved"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "idle_co2_grams", "co2_grams") or
        _div(df["idle_co2_grams"].sum(), df["co2_grams"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "payload_tonnes") or
        round(float(df["payload_tonnes"].sum()), 1)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "payload_tonnes", "fuel_litres") or
        _div(df["payload_tonnes"].sum(), df["fuel_litres"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "overloaded_loads", "load_count") or
        _div(df["overloaded_loads"].sum(), df["load_count"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "haul_distance_km", "trip_count") or
        _div(df["haul_distance_km"].sum(), df["trip_count"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "engine_on_hours", "speeding_hours") or
        _div(
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "route_deviations", "trip_count") or
        _div(df["route_deviations"].sum(), df["trip_count"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "geofence_violations", "trip_count") or
        _div(df["geofence_violations"].sum(), df["trip_count"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "queue_time_min", "cycle_time_min") or
        _div(df["queue_time_min"].sum(), df["cycle_time_min"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "pre_shift_checks_done", "pre_shift_checks_due") or
        _div(df["pre_shift_checks_done"].sum(), df["pre_shift_checks_due"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "operators_licensed", "operators_total") or
        _div(df["operators_licensed"].sum(), df["operators_total"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "training_completed_count", "training_required_count") or
        _div(df["training_completed_count"].sum(), df["training_required_count"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "incidents_reported", "incidents_total") or
        _div(df["incidents_reported"].sum(), df["incidents_total"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "open_work_orders", "target_work_orders") or
        _div(df["open_work_orders"].sum(), df["target_work_orders"].sum())
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "work_orders_created", "defects_raised") or
        _div(df["work_orders_created"].sum(), df["defects_raised"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "parts_available", "parts_required") or
        _div(df["parts_available"].sum(), df["parts_required"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "correct_predictions", "total_predictions") or
        _div(df["correct_predictions"].sum(), df["total_predictions"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "anomalies_confirmed", "anomalies_detected") or
        _div(df["anomalies_confirmed"].sum(), df["anomalies_detected"].sum(), 100)
    ),
))
...
_register(KPIDefinition(
    ...
    compute=lambda df: (
        _require_cols(df, "recommendations_adopted", "recommendations_total") or
        _div(df["recommendations_adopted"].sum(), df["recommendations_total"].sum(), 100)
    ),
))
