...
Note: the CSV Vectorization Pipeline's Stage-1 schema inspection
...
from data_analysis_agent.tools.column_mapper import (
    map_columns_to_kpi_variables,
    map_direct_kpi_columns,
)
...
def discover_schema(file_path: str, backend: str = "bedrock") -> dict:
    ...
    backend : str
        LLM backend for column mapping — "bedrock" (default) or "ollama".
    ...
    column_mapping = map_columns_to_kpi_variables(schema, backend=backend)

    # Ask the LLM which columns ARE already a pre-computed KPI value.
    direct_kpi_mapping = map_direct_kpi_columns(schema, backend=backend)
