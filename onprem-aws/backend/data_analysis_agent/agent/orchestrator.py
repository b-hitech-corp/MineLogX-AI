...
try:
    ...
    raise ImportError(
        "strands-agents is required for the Ollama orchestrator. "
        "Install it with: pip install strands-agents strands-agents-tools\n"
        f"Original error: {_e}"
    ) from _e
...
def schema_advisor__discover_schema(file_path: str) -> dict:
    ...
    """
    return schema_advisor.discover_schema(file_path=file_path, backend="ollama")
