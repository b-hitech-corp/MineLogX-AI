...
import tools.chart_spec_builder as chart_spec_builder
import tools.csv_loader as csv_loader
import tools.insight_extractor as insight_extractor
import tools.kpi_engine as kpi_engine
import tools.schema_advisor as schema_advisor
import tools.stats_analyzer as stats_analyzer
    ...
    def _build_dashboard(self, files_data: list[dict]) -> dict:
        ...
        for fd in files_data:
            ...
            for kpi in (fd.get("kpis") or {}).get("feasible") or []:
                section = _KPI_TO_SECTION.get(kpi.get("name", ""))
                if section:
